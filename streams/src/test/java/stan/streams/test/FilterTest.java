    ...
    extends MainTest
{
    ...
    {
        int sum1 = 0;
        for(int it: data) if(it > 250 && it < 750)
            sum1 += (it + "_" + it*2 +"_" + it/2 + "_" + it*it).hashCode();
        int sum2 = 0;
        for(int it: Streams.from(data).filter(new Function<Integer, Boolean>()
        {
        ...
        }).turn(To.<Integer>list())) sum2 += (it + "_" + it*2 + "_" + it/2 + "_" + it*it).hashCode();
        assertEquals("Sums must be equals!", sum1, sum2);
    }
    private void filterStringList(List<String> data)
    {
        int sum1 = 0;
        for(String it: data) if(it.length() > 75 && it.length() < 125)
            sum1 += (it + "_" + it.length()*2 + "_" + it.length()/2
                + "_" + it.length()*it.length()).hashCode();
        int sum2 = 0;
        for(String it: Streams.from(data).filter(new Function<String, Boolean>()
        {
            ...
            sum2 += (it + "_" + it.length()*2 + "_" + it.length()/2
                + "_" + it.length()*it.length()).hashCode();
        assertEquals("Sums must be equals!", sum1, sum2);
    }
    ...
    {
        int sum1 = 0;
        for(Object it: data) if((it.hashCode()/3-1)%2 == 0)
            sum1 += (it.hashCode() + "_" + it.hashCode()*2 + "_" + it.hashCode()/2
                + "_" + it.hashCode()*it.hashCode()).hashCode();
        int sum2 = 0;
        for(Object it: Streams.from(data).filter(new Function<Object, Boolean>()
        {
            ...
            sum2 += (it.hashCode() + "_" + it.hashCode()*2 + "_" + it.hashCode()/2
                + "_" + it.hashCode()*it.hashCode()).hashCode();
        assertEquals("Sums must be equals!", sum1, sum2);
    }
}